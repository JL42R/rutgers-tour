...
// getting close. Each NPC is a floating scarlet badge (drawn on a canvas,
// no image files needed). Walk within its radius -> "Press E to talk"
  ...
  setNPCs(list) {
    ...
    this.npcs = list.map((data) => {
      const sprite = new THREE.Sprite(new THREE.SpriteMaterial({
        map: makeBadgeTexture((data.name ?? '?')[0].toUpperCase()),
        depthTest: true
      }));

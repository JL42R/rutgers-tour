...
- **Raw `.ply` and raw capture video never enter Git.** The raw printing-room export is ~240 MB, over GitHub's 100 MB hard limit. `.gitignore` excludes `*.ply` with a per-file negation for the finished compressed zone. Raw captures live in the Drive folder linked in `docs/ONBOARDING.md`.
